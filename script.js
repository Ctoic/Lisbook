...
function showErrorPopup(message) {
  ...
  errorPopup.style.borderRadius = "8px"; // Added rounded corners
  errorPopup.style.textAlign = "center";
  errorPopup.innerHTML = `
    <h2 style="color: #c0392b; font-weight: 600; margin-bottom: 10px;">Attention</h2>
    <p>${message}</p>
    <button style="margin-top: 15px; padding: 8px 15px; background-color: #3498db; color: white; border: none; border-radius: 4px; cursor: pointer;">Close</button>
  `;
...
// Function to load an HTML file into an element (Note: File loading may not work
// in all environments if external files like header.html are not available.)
...
const toggleTheme = () => {
  ...
  if (newTheme === "light") {
    if (img1) img1.src = "./Images/lisbook-about-white.jpg";
    if (img2) img2.src = "./Images/lisbook-about-white.jpg";
  } else {
    if (img1) img1.src = "./Images/lisbook-about.jpg";
    if (img2) img2.src = "./Images/lisbook-about.jpg";
  }
};
...
async function fetchContributors(repoOwner, repoName) {
  try {
    ...
    if (!response.ok) {
      // Throw a specific error if the repo is private or doesn't exist
    ...
    // You could call showErrorPopup here if the user should see the error
    return null;
  }
}
...
async function loadContributors() {
  ...
  if (contributors && contributors.length > 0) {
    ...
    if (container) {
      ...
      // Optional: Display a message if fetching failed but the container exists
      const container = document.querySelector(".row.row-cols-1.row-cols-md-3.row-cols-lg-5.g-4.mt-4.align-items-stretch");
      if (container) {
        container.innerHTML = `<p class="text-center text-danger">Failed to load contributors from GitHub. Check console for details.</p>`;
      }
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function initializeTheme() {
    ...
    toggleTheme(currentTheme); // Run once to set images
  }
  ...
  if (editNameBtn) {
    editNameBtn.addEventListener("click", () => {
      // NOTE: prompt() is generally discouraged. Replace with a custom modal for input.
...
// Call the contributor loading function when the page loads
